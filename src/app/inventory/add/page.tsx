...
export default function AddInventoryPage() {
  ...
  const { user, allBranches, selectedBranchId, setIsSyncing, showToast } = usePCMSStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.branch && user?.allAccess) {
      showToast('Please select a clinical site (branch) first.', 'error');
      return;
    }
    ...
    <div className="animate-fade-in" style={{ paddingBottom: '5rem' }}>
      ...
      <div className="clinical-form-card" style={{ margin: '0 auto', padding: '3rem' }}>
        ...
        <form onSubmit={handleSubmit}>
          {user?.allAccess && (
            <div style={{ marginBottom: '2rem', padding: '1.5rem', background: 'var(--bg-subtle)', borderRadius: 'var(--radius-md)', border: '1px dashed var(--primary)' }}>
              <label className="label-premium" style={{ color: 'var(--primary)' }}>Clinical Site / Branch</label>
              <div style={{ position: 'relative' }}>
                <MapPin size={18} style={{ position: 'absolute', left: '1.25rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--primary)' }} />
                <select 
                  required 
                  className="input-premium" 
                  style={{ paddingLeft: '3.5rem', borderColor: 'var(--primary)' }} 
                  value={formData.branch} 
                  onChange={e => setFormData({...formData, branch: e.target.value})}
                >
                  <option value="">Select Branch for this Inventory Item</option>
                  {allBranches.map(b => <option key={b._id} value={b._id}>{b.name}</option>)}
                </select>
              </div>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>Specify which clinic stock this item belongs to.</p>
            </div>
          )}
