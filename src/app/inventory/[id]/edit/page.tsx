...
export default function EditInventoryPage() {
  ...
  const { user, allBranches, setIsSyncing, showToast } = usePCMSStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.branch && user?.allAccess) {
        showToast('Please specify the clinical site for this item.', 'error');
        return;
    }
    setLoading(true);
    setIsSyncing(true);
        ...
        <form onSubmit={handleSubmit}>
          {user?.allAccess && (
            <div style={{ marginBottom: '2rem', padding: '1.5rem', background: 'var(--bg-subtle)', borderRadius: 'var(--radius-md)', border: '1px dashed var(--primary)' }}>
              ...
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>Update the linked clinical site if required.</p>
            </div>
          )}
