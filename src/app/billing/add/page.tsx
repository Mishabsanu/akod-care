  ...
  return (
    <div className="create-invoice-container animate-fade-in" style={{ maxWidth: '900px', paddingBottom: '5rem' }}>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Generate professional medical invoices for patients across all clinical sites.</p>
      </div>

      <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 320px', gap: '2rem' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div className="card" style={{ padding: '2rem' }}>
                <h3 style={{ fontSize: '0.9rem', fontWeight: 700, marginBottom: '1.5rem', color: 'var(--text-muted)' }}>PATIENT & SITE</h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                    ...
                    <div>
                        <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, marginBottom: '0.5rem' }}>Clinical Branch</label>
                        <select required value={formData.branch} onChange={(e) => setFormData({...formData, branch: e.target.value})} style={{ width: '100%', padding: '0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)', background: 'white' }}>
