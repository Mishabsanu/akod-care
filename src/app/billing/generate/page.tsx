...
export default function GenerateInvoicePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    items: [{ name: '', quantity: 1, price: 0, serviceId: '', inventoryId: '' }],
    status: 'Unpaid',
    discount: 0,
    amount: 0,
    remarks: ''
  ...
  // -------------------------------------------------------------------
  // SYNC | Fetch Clinical Registry & Service Catalog
  // -------------------------------------------------------------------
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const amount = Math.max(0, subtotal - formData.discount);
  ...
  const validateForm = () => {
    ...
    if (!formData.branch) {
      showToast('Please select a clinical branch.', 'error');
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        branchName: selectedBranch?.name || 'Main',
      ...
      const detailedMsg = err.response?.data?.details ? ` (${err.response.data.details})` : '';
      showToast(`${errorMsg}${detailedMsg}`, 'error');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div style={{ marginBottom: '3rem', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <button
            type="button"
            onClick={() => router.back()}
            style={{ marginBottom: '1rem', color: 'var(--text-muted)', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 600, transition: 'var(--transition-smooth)' }}
          >
            ← Back to Billing
          </button>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', alignItems: 'flex-end' }}>
                      {user?.allAccess && (
                        <div style={{ width: '220px' }}>
                          <label className="label-premium" style={{ fontSize: '0.65rem' }}>Authorized Branch Site</label>
                          <select required className="input-premium" style={{ height: '42px', fontSize: '0.9rem', fontWeight: 600 }} value={formData.branch} onChange={(e) => setFormData({ ...formData, branch: e.target.value })}>
                    ...
                    {formData.items.map((item, idx) => (
                      <tr key={idx} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                        ...
                        <td style={{ padding: '1rem 0.5rem' }}>
                          <input required type="number" className="input-premium" style={{ textAlign: 'center', fontSize: '0.9rem' }} value={item.quantity} onChange={(e) => {
                            const newItems = [...formData.items];
                        ...
                        <td style={{ padding: '1rem 0.5rem' }}>
                          <div style={{ position: 'relative' }}>
                            ...
                            <input required type="number" className="input-premium" style={{ paddingLeft: '1.75rem', textAlign: 'right', fontSize: '0.9rem', fontWeight: 600 }} value={item.price} onChange={(e) => {
                              const newItems = [...formData.items];
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                      ...
                      <div>
                        <h4 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-main)', margin: 0, letterSpacing: '-0.01em' }}>
                          {selectedBranch?.name || 'Authorized Clinic Site'}
                        </h4>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                        <MapPin size={12} /> {selectedBranch?.address || 'Branch Registry Address'}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                        <Phone size={12} /> {selectedBranch?.phone || 'Emergency Line N/A'}
                      </div>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '4rem', marginTop: '1.5rem' }}>
                  <div>
                    ...
                    <div style={{ marginTop: '3rem' }}>
                      <div style={{ height: '1px', width: '180px', background: '#cbd5e1', marginBottom: '0.5rem' }}></div>
                      <p style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-main)', letterSpacing: '0.02em' }}>Clinical Authorization</p>
                      <p style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>Automated Clinical ID Validation System</p>
                    </div>
                  </div>
