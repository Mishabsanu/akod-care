...
export default function InvoiceDetailsPage() {
  ...
  const handleDownloadPDF = () => {
    ...
    generateInvoicePDF({
      ...
      branchName: invoice.branch?.name || 'Main',
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-[80vh] gap-4">
      ...
      <p className="font-bold text-slate-400 animate-pulse text-xs tracking-widest">🛡️ ACCESSING FINANCIAL LEDGER...</p>
    </div>
  );
        ...
        <div className="col-8">
          <div className="clinical-form-card" style={{ padding: '2.5rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', padding: '2rem', background: '#f8fafc', borderRadius: 'var(--radius-lg)', marginBottom: '3rem', border: '1.5px solid var(--border-subtle)' }}>
              ...
              <div>
                <label className="label-premium" style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Building size={14} /> ISSUING BRANCH</label>
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                   <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#e2e8f0', color: '#64748b', display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Building size={20} /></div>
                   <div>
                     <p style={{ margin: 0, fontWeight: 850, fontSize: '1rem' }}>{invoice.branch?.name || 'Main Branch'}</p>
                     <p style={{ margin: 0, fontSize: '0.8rem', opacity: 0.6 }}>Authorized Site Ledger</p>
                   </div>
                </div>
              </div>
            </div>
          ...
          <div style={{ padding: '1.5rem', borderRadius: 'var(--radius-md)', background: 'rgba(15, 118, 110, 0.05)', border: '1px dashed var(--primary)' }}>
            <div style={{ display: 'flex', gap: '1rem' }}>
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', lineHeight: 1.6, margin: 0 }}>
                Clinical invoices are <strong>Financial Immutables</strong>. Any changes made to the status or amount will be logged for branch-level revenue reconciliation.
              </p>
            </div>
          </div>
        </div>
