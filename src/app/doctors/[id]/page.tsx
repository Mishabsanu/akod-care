    ...
    <div className="doctor-details-container animate-fade-in clinical-form-wide" style={{ paddingBottom: '7rem' }}>
      ...
      <div style={{ marginBottom: '3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Detailed medical credentials, assigned clinical branches, and operational status.</p>
        </div>
        <button
          ...
          <div className="clinical-form-card" style={{ height: '100%' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', marginTop: '3rem' }}>
              ...
              <div className="spec-block" style={{ gridColumn: 'span 2' }}>
                <label className="label-premium" style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Building2 size={14} style={{ color: 'var(--primary)' }} /> CLINICAL SITE ASSIGNMENTS
                </label>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '1rem' }}>
                  {doctor.branches?.length > 0 ? doctor.branches.map((b: any) => (
                    <div key={b._id} style={{ padding: '1rem', background: 'white', border: '1.5px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', display: 'flex', alignItems: 'center', gap: '0.75rem', fontWeight: 800, fontSize: '0.9rem' }}>
                      <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--primary)' }} />
                      {b.name}
                    </div>
                  )) : (
                    <div style={{ gridColumn: 'span 2', padding: '2rem', border: '2px dashed var(--border-subtle)', borderRadius: 'var(--radius-md)', textAlign: 'center', color: 'var(--text-muted)', fontWeight: 700 }}>
                      No specific site assignments. System defaults to Global Access.
                    </div>
                  )}
                </div>
              </div>
          ...
          <div style={{ padding: '2rem', background: 'linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 100%)', borderRadius: 'var(--radius-lg)', color: 'white', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              ...
              <p style={{ fontSize: '0.85rem', lineHeight: 1.6, fontWeight: 600, opacity: 0.9 }}>
                Specialist assignments affect patient scheduling availability. Ensure the "Assigned Branch" remains accurate to prevent site-level scheduling conflicts.
              </p>
            </div>
          </div>
        </div>
