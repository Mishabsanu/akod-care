...
import { ArrowLeft, Stethoscope, Phone, Mail, Building, Activity, MessageCircle, CheckCircle2 } from 'lucide-react';
...
export default function EditDoctorPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsSyncing(true);
      try {
        const [docRes, branchesRes] = await Promise.all([
          api.get(`/doctors/${id}`),
          api.get('/branches')
        ]);
        ...
        if (doctor) {
          setFormData({
            ...
            phone: doctor.phone,
            email: doctor.email || '',
            branch: doctor.branches?.[0]?._id || doctor.branch?._id || doctor.branch || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      payload.branches = [formData.branch]; // Backend consistency
  ...
  return (
    <div className="edit-doctor-container animate-fade-in clinical-form-wide" style={{ paddingBottom: '7rem' }}>
      ...
      <div style={{ marginBottom: '3rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Update medical specialist profile, site assignment, and operational status.</p>
      </div>
          ...
          <div className="col-4">
            ...
            <div style={{ position: 'relative' }}>
              ...
              <select 
                ...
                style={{ paddingLeft: '2.75rem', background: !user?.allAccess ? '#f8fafc' : 'white' }} 
