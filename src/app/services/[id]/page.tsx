    ...
    <div className="service-details-container animate-fade-in clinical-form-wide" style={{ paddingBottom: '7rem' }}>
      ...
      <div style={{ marginBottom: '3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Detailed clinical specifications, pricing, and site availability for this modality.</p>
        </div>
        <button
          ...
          <div className="clinical-form-card" style={{ height: '100%' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2.5rem', marginTop: '2rem' }}>
              ...
              <div className="spec-item" style={{ gridColumn: 'span 2' }}>
                <label className="label-premium" style={{ marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <ShieldCheck size={14} style={{ color: 'var(--primary)' }} /> BRANCH AVAILABILITY
                </label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginTop: '0.5rem' }}>
                  {service.branches?.length > 0 ? service.branches.map((b: any) => (
                    <div key={b._id} style={{ padding: '0.6rem 1rem', background: '#f8fafc', border: '1.5px solid var(--border-subtle)', borderRadius: 'var(--radius-sm)', fontSize: '0.85rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <Building2 size={14} style={{ opacity: 0.5 }} /> {b.name}
                    </div>
                  )) : (
                    <div style={{ color: 'var(--text-muted)', fontWeight: 600, fontSize: '0.85rem', background: '#f8fafc', padding: '1rem', borderRadius: 'var(--radius-sm)', width: '100%', border: '1px dashed' }}>
                      Global Access - Available at all clinical sites.
                    </div>
                  )}
                </div>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', background: '#f8fafc', borderRadius: 'var(--radius-sm)' }}>
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '0.8rem', fontWeight: 800 }}>USAGE CATEGORY</span>
                </div>
                <span style={{ fontSize: '0.85rem', fontWeight: 700 }}>Clinical Modal</span>
              </div>
            </div>
            ...
            <div style={{ marginTop: '2.5rem', padding: '1.25rem', background: 'rgba(15, 118, 110, 0.05)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--primary)' }}>
              <div style={{ display: 'flex', gap: '1rem' }}>
                ...
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', lineHeight: 1.6, margin: 0 }}>
                  This modality profile is <strong>Authoritative</strong>. Prices and branch links are synchronized across the financial and scheduling modules. Use edits sparingly to maintain billing consistency.
                </p>
              </div>
            </div>
          </div>
