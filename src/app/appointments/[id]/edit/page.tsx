...
export default function EditAppointmentPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsSyncing(true);
      try {
        const [appRes, patientsRes, doctorsRes, branchesRes] = await Promise.all([
        ...
        if (appointment) {
          ...
          const resolvedBranchId = 
            (typeof appointment.branch === 'object' && appointment.branch !== null) 
              ? (appointment.branch._id || appointment.branch.id) 
              : appointment.branch;
