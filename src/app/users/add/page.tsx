...
export default function OnboardUserPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation: Non-owners must have a branch
    const selectedRole = roles.find(r => r._id === formData.roleId);
    if (selectedRole && !selectedRole.allAccess && !formData.branchId) {
      showToast('Please assign a branch to this staff member.', 'error');
      return;
    }
        ...
        <div className="clinical-form-grid">
          ...
          {user?.allAccess && (
            <div className="col-6">
              ...
              <select disabled={loading} className="input-premium" value={formData.branchId} onChange={(e) => setFormData({ ...formData, branchId: e.target.value })}>
