  ...
  return (
    <div className="edit-user-container animate-fade-in clinical-form-wide" style={{ paddingBottom: '7rem' }}>
      ...
      <div style={{ marginBottom: '3rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Adjust system access permissions, clinical site assignment, and account status.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="clinical-form-card" style={{ opacity: saving ? 0.7 : 1 }}>
        ...
        <div className="clinical-form-grid">
          ...
          {currentUser?.allAccess && (
            <div className="col-6">
              <label className="label-premium">Assigned Branch Context</label>
              <div style={{ position: 'relative' }}>
                <Building size={16} style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)', opacity: 0.5 }} />
                <select disabled={saving} className="input-premium" style={{ paddingLeft: '2.75rem' }} value={formData.branchId} onChange={(e) => setFormData({...formData, branchId: e.target.value})}>
                  <option value="">{roles.find(r => r._id === formData.roleId)?.allAccess ? 'Global Access (No site lock)' : 'Select Clinical Branch'}</option>
