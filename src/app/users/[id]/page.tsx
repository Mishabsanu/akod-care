...
export default function UserDetailsPage() {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-[80vh] gap-4">
      ...
      <p className="font-bold text-slate-400 animate-pulse text-xs tracking-widest">🛡️ AUTHENTICATING ACCESS REGISTRY...</p>
    </div>
  );
    ...
    <div className="user-details-container animate-fade-in clinical-form-wide" style={{ paddingBottom: '7rem' }}>
      ...
      <div style={{ marginBottom: '3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Detailed breakdown of system user credentials, clinical roles, and branch-level permissions.</p>
        </div>
        <button
          ...
          <div className="clinical-form-card" style={{ height: '100%' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', marginTop: '3rem' }}>
              ...
              <div className="spec-block">
                <label className="label-premium" style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Building2 size={14} style={{ color: 'var(--primary)' }} /> CLINICAL SITE ASSIGNMENT
                </label>
                <div style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--text-main)' }}>
                  {userData.branch?.name || 'Global Access'}
                </div>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600, marginTop: '0.25rem' }}>Primary operational branch</p>
              </div>

              <div className="spec-block">
                <label className="label-premium" style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Key size={14} style={{ color: 'var(--primary)' }} /> SYSTEM IDENTITY
                </label>
                <div style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--text-main)' }}>
                   UUID-{String(id).slice(-8).toUpperCase()}
                </div>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600, marginTop: '0.25rem' }}>Immutable system reference</p>
              </div>
            </div>
          </div>
        </div>
