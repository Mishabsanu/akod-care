                ...
                <div>
                    ...
                    <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', opacity: 0.7 }}>
                        ...
                        <span style={{ fontWeight: 700, fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                            <MapPin size={14} /> {patient.branch?.name?.toUpperCase()}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(350px, 1fr) 2fr', gap: '1.5rem' }}>
                ...
                <aside style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div className="card-premium" style={{ padding: '1.75rem', background: 'white' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                            <h3 style={{ fontSize: '1.1rem', fontWeight: 900, margin: 0 }}>CLINICAL VITALS</h3>
                            <TrendingUp size={20} style={{ color: 'var(--primary)' }} />
                        </div>
                        
                        <div style={{ display: 'grid', gap: '1.5rem' }}>
                            <div style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)', padding: '1.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                                    <span style={{ fontSize: '0.7rem', fontWeight: 900, color: 'var(--text-muted)', letterSpacing: '0.05em' }}>BODY MASS INDEX</span>
                                    <span style={{ fontSize: '0.65rem', fontWeight: 800, background: '#dcfce7', color: '#10b981', padding: '0.2rem 0.6rem', borderRadius: '1rem' }}>HEALTHY</span>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                                    <span style={{ fontSize: '2rem', fontWeight: 950, color: 'var(--text-main)', letterSpacing: '-0.03em' }}>{patient.bmi || '24.2'}</span>
                                    <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)' }}>kg/m²</span>
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                                <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                                        ...
                                        <span style={{ fontSize: '0.7rem', fontWeight: 900, color: 'var(--text-muted)' }}>WEIGHT</span>
                                    </div>
                                    <span style={{ fontSize: '1.5rem', fontWeight: 900 }}>{patient.weight} <small style={{ fontSize: '0.7rem', opacity: 0.5 }}>KG</small></span>
                                </div>
                                <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                                        <Ruler size={14} style={{ color: 'var(--primary)' }} />
                                        <span style={{ fontSize: '0.7rem', fontWeight: 900, color: 'var(--text-muted)' }}>HEIGHT</span>
                                    </div>
                                    <span style={{ fontSize: '1.5rem', fontWeight: 900 }}>{patient.height} <small style={{ fontSize: '0.7rem', opacity: 0.5 }}>CM</small></span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="card-premium" style={{ padding: '1.75rem', background: 'white' }}>
                        <h3 style={{ fontSize: '1.1rem', fontWeight: 900, marginBottom: '2rem' }}>CLINICAL HABITS</h3>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem' }}>
                            {patient.habits?.length > 0 ? patient.habits.map(h => (
                                <span key={h} style={{ padding: '0.5rem 1rem', background: 'var(--border-subtle)', borderRadius: '0.75rem', fontSize: '0.8rem', fontWeight: 700 }}>{h}</span>
                            )) : <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>No habits recorded.</p>}
                        </div>
                    </div>
                </aside>
                ...
                <main style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div className="card-premium" style={{ padding: '2rem', background: 'white' }}>
                        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginBottom: '1.5rem' }}>
                            ...
                            <h3 style={{ fontSize: '1.3rem', fontWeight: 950, letterSpacing: '-0.02em' }}>PRIMARY CLINICAL COMPLAINT</h3>
                        </div>
                        <p style={{ background: 'rgba(15, 118, 110, 0.03)', borderLeft: '4px solid var(--primary)', padding: '1.5rem', fontSize: '1rem', fontWeight: 600, color: 'var(--text-main)', lineHeight: 1.8, borderRadius: '0 1rem 1rem 0' }}>
                    ...
                    <div className="card-premium" style={{ padding: '2rem', background: 'white' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
                             <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                                ...
                                <h3 style={{ fontSize: '1.3rem', fontWeight: 950, letterSpacing: '-0.02em' }}>RECENT TREATMENT NOTES</h3>
                             </div>
                             <button onClick={() => setActiveTab('timeline')} style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                                VIEW FULL HISTORY <ChevronRight size={16} />
                             </button>
                        </div>
                        
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            {patient.treatments?.slice(0, 3).map((t, i) => (
                                <div key={i} style={{ borderBottom: '1.5px solid var(--border-subtle)', paddingBottom: '1.5rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                                        <span style={{ fontSize: '0.85rem', fontWeight: 900 }}>{new Date(t.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                        <span style={{ fontSize: '0.7rem', fontWeight: 800, opacity: 0.5 }}>SESSION #{patient.treatments.length - i}</span>
                                    </div>
                                    <p style={{ fontSize: '1rem', color: 'var(--text-main)', fontWeight: 500, lineHeight: 1.7 }}>{t.notes}</p>
                                </div>
                            ))}
              ...
              <div className="animate-fade-in">
                 <div className="card-premium" style={{ padding: '1.75rem', background: 'white' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
                        ...
                        <button onClick={() => router.push('/billing/generate')} style={{ padding: '0.6rem 1.25rem', borderRadius: 'var(--radius-md)', background: 'var(--primary)', color: 'white', fontWeight: 800, fontSize: '0.8rem' }}>NEW INVOICE</button>
                    </div>
