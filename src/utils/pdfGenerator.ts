...
export const generateInvoicePDF = (data: InvoiceData) => {
  ...
  // -------------------------------------------------------------------
  // 🎭 BRANDING | Minimalist Header
  // -------------------------------------------------------------------
  const lx = 25;
  const ly = 25;
  
  // Minimalist Logo (Outline)
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(20);
  doc.text('PHYSIO CLINIC', 42, 23);
  
  doc.setTextColor(...textMuted);
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.text('PREMIUM CARE & CLINICAL REHABILITATION', 42, 29, { charSpace: 0.5 });
  ...
  doc.setFontSize(7);
  doc.setFont('helvetica', 'bold');
  doc.text('CLINICAL SITE', 20, startY);
  
  doc.setTextColor(...textMain);
  doc.setFontSize(10);
  doc.text(data.branchName.toUpperCase(), 20, startY + 6);
  doc.setTextColor(...textMuted);
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  const splitAddress = doc.splitTextToSize(data.branchAddress || 'Main Registry', 65);
  doc.text(splitAddress, 20, startY + 11);
  doc.text(`Contact: ${data.branchPhone || 'N/A'}`, 20, startY + 20);
  ...
  if (data.id) {
     ...
     doc.text('This clinical invoice is generated by the PCMS automated billing engine.', 20, authY + 4);
  ...
  doc.setFontSize(7);
  doc.setFont('helvetica', 'bold');
  doc.text('CLINICAL AUTHORIZATION', 167.5, authY + 20, { align: 'center' });
