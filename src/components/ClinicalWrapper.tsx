...
export default function ClinicalWrapper({ children }: { children: React.ReactNode }) {
  ...
  const { user, selectedBranchId, fetchInitialData, isLoading, isSyncing } = usePCMSStore();
  ...
  return (
    <div className="main-wrapper">
      ...
      <main className="content-area">
        <Header />
        <div key={user?.id + (selectedBranchId || 'global')} className="page-content animate-fade-in">
            {children}
        </div>
      </main>
      <GlobalToast />
