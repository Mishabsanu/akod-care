...
const ALL_MODULES = [
  ...
  { label: 'Branch Network', href: '/branches', icon: Building2, color: '#64748b' },
  { label: 'Roles & Access', href: '/roles', icon: Lock, color: '#64748b' }
];

export default function Header() {
  const { user, selectedBranchId, allBranches, setSelectedBranchId } = usePCMSStore();
  ...
  const activeBranch = allBranches.find(b => b._id === selectedBranchId);
  ...
  return (
    <header className="top-nav animate-fade-in">
      <div style={{ display: 'flex', gap: '2rem', alignItems: 'center', flex: 1 }}>
        ...
        {/* Clinical Switchboard | Premium Multi-Branch Control */}
        {user.allAccess && (
          <div style={{ position: 'relative' }}>
            <button 
              onClick={() => setIsOpen(!isOpen)}
              className={`glass-interactive ${activeBranch ? 'pulse-site' : ''}`} 
              style={{ 
            ...
            >
              <Globe size={18} />
              {activeBranch ? activeBranch.name.toUpperCase() : 'ALL CLINICAL SITES'}
              <ChevronDown size={16} style={{ opacity: 0.5, marginLeft: '0.5rem', transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.3s' }} />
            </button>

            {isOpen && (
              <div className="glass-dark switchboard-overlay animate-fade-in" style={{ width: '520px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                    <div>
                        <h4 style={{ color: 'white', fontSize: '1rem', fontWeight: 800, letterSpacing: '-0.01em' }}>CLINICAL SWITCHBOARD</h4>
                        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.05em' }}>SELECT OPERATIONAL SITE</p>
                    </div>
                    <button 
                ...
                <div style={{ marginBottom: '1.25rem', position: 'relative' }}>
                    ...
                    <input 
                        type="text" 
                        placeholder="Filter clinical locations..." 
                        className="switchboard-search"
                ...
                <div className="switchboard-grid">
                  {/* Global Oversight Toggle */}
                  <div 
                    onClick={() => { setSelectedBranchId(''); setIsOpen(false); }}
                    className={`switchboard-item ${!selectedBranchId ? 'active' : ''}`}
                    style={{ gridColumn: 'span 2', display: 'flex', alignItems: 'center', gap: '1rem' }}
                  >
                    <div style={{ 
                      width: '40px', 
                      height: '40px', 
                    ...
                    <div>
                        <p style={{ color: 'white', fontSize: '0.85rem', fontWeight: 700 }}>GLOBAL OVERSIGHT</p>
                        <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.65rem' }}>Aggregated data view across all branches</p>
                    </div>
                  </div>
                  ...
                  {filteredBranches.map(branch => (
                    <div 
                      key={branch._id} 
                      onClick={() => { setSelectedBranchId(branch._id); setIsOpen(false); }}
                      className={`switchboard-item ${selectedBranchId === branch._id ? 'active' : ''}`}
                      style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                        <Building2 size={14} style={{ color: 'rgba(255,255,255,0.5)' }} />
                        <p style={{ color: 'white', fontSize: '0.85rem', fontWeight: 700, margin: 0 }}>{branch.name}</p>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginTop: '0.25rem' }}>
                         <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#22c55e' }} />
                         <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.02em' }}>OPERATIONAL</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {filteredBranches.length === 0 && (
                   <div style={{ textAlign: 'center', padding: '3rem 1rem', color: 'rgba(255,255,255,0.3)' }}>
                      ...
                      <p style={{ fontSize: '0.85rem' }}>No matching clinical sites found.</p>
                   </div>
                )}
              </div>
            )}
        ...
        <div className="user-profile" style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', padding: '0.5rem', paddingLeft: '1.5rem', background: 'white', borderRadius: 'var(--radius-lg)', border: '2px solid var(--border-subtle)', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
          <div className="profile-details" style={{ textAlign: 'right' }}>
            ...
            <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: 800, margin: 0, opacity: 0.8 }}>
              {user?.roleName} • {activeBranch?.name || 'CENTRAL OPS'}
            </p>
          </div>
