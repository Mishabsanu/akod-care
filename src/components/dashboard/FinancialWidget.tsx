...
export default function FinancialWidget({ filterParams }: { filterParams: any }) {
    const { selectedBranchId, user } = usePCMSStore();
    ...
    const { summary, trends, branchComparison } = data || {};
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
            ...
            {/* Sub Widgets (Graph and Branch Comparison) */}
            <div style={{ display: 'grid', gridTemplateColumns: user?.allAccess && !selectedBranchId ? '2fr 1fr' : '1fr', gap: '1.5rem' }}>
                ...
                {/* Multi-Branch Global Comparison Panel */}
                {user?.allAccess && !selectedBranchId && (
                    <div className="card" style={{ padding: '2rem', borderLeft: '4px solid #6366f1' }}>
                        <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '2rem' }}>Network <span className="gradient-text">Performance</span></h3>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            {branchComparison?.length === 0 ? (
                                <div style={{ textAlign: 'center', opacity: 0.5, fontSize: '0.8rem' }}>No network activity found.</div>
                            ) : (
                                branchComparison?.sort((a: any, b: any) => b.revenue - a.revenue).map((b: any, i: number) => (
                                    <div key={b.name} style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '0.75rem', borderBottom: i !== branchComparison.length - 1 ? '1px dashed #f1f5f9' : 'none' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            <span style={{ fontSize: '0.8rem', fontWeight: 800 }}>{b.name}</span>
                                            <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>Branch Location</span>
                                        </div>
                                        <span style={{ fontSize: '0.9rem', fontWeight: 900, color: '#6366f1' }}>₹{b.revenue.toLocaleString()}</span>
                                    </div>
                                ))
                            )}
                        </div>
